...
class client extends connect{
    ...
    private $queryDelete = 'DELETE tb_client WHERE identificacion = :cc';
    private $message;
    use getInstance;
    function __construct(private $identificacion, public $full_name, public $email, private $address, private $phone){parent::__construct();}
    ...
    public function deleteClient(){
        try {
            ...
            if ($res->rowCount() > 0) {
                $this->message = ["Code" => 200, "Message" => "Record deleted"];
            } else {
                $this->message = ["Code" => 404, "Message" => "No matching record found"];
            }
        } catch(\PDOException $e) {
